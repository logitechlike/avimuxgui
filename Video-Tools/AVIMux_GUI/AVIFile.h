...
/*
  F黵 diesen Quellcode gilt die GPL

  
  Klassen f黵 Zugriff auf AVI-Dateien, incl. OpenDML > 4 GB
  ...
  Status:
    AVI lesen:
      Standard-AVI:
	      OK
	  OpenDML-AVI:
	      Index: Super-, Standard- und 2-Field-Index unterst黷zt
			       2-Field-Index nicht getestet
		  timecode-, timecode-discontinuity-table: N/A
		  Dateien > 4 GB (NTFS): funktionsf鋒ig
	  Videodekompression: beta
		  unkomprimierte Videos werden nicht unterst黷zt !
		  8-Bit-Daten werden nicht unterst黷zt !
		  Paletten werden nicht unterst黷zt
		  DirectStreamCopy: OK
		  Seeken: beliebig
	  Audiodekompression: beta
		  Audiostreams konstanter Datenrate: i.O.
		  Audiostreams mit Blockgr鲞en > 1 Byte: nicht getestet
		  Audiostreams variabler Datenrate: (nur Direct Stream Copy) Chunkweises lesen m鰃lich
		  DirectStreamCopy: OK
		  Seeken: beliebig
	...
	RAM-Bedarf f黵 Index:
	  28 Bytes / Chunk, d.h. 41 kByte/min = 2,4 MB/h f黵 Video @ 25 fps
	                                      = 10,9 MB/h f黵 Video @ 25 fps + 2x MP3-VBR-Audio
...
public:
	...
	__int64					qwStreamLength; // gesamte L鋘ge des Streams
	...
	void*					lpBufIn;		// nur Audio: Eingabepuffer f黵 Audiodekompr.
	void*					lpBufOut;		// nur Audio: Ausgabepuffer f黵 Audiodekompr.
...
// Header, die f黵 OpenDML n鰐ig sind
...
// wenn wir mal einen Framecache f黵 bereits dekomprimierte Frames bauen wollen...
struct FRAMEINFO {
	DWORD dwAccCount;
	DWORD iFrameNbr;
};
