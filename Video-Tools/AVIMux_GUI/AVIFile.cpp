...
//			lstrcat(Buffer, " - Debug-Messages.txt");
			...
			{
/*				wsprintf(Buffer, "Datei %s konnte nicht ge鰂fnet werden",
					Buffer);
				MessageBox(0,Buffer,"Fehler",MB_OK); */
				SetDebugState(DS_DEACTIVATE);
			}
		}
		DebugMsg("Datei ge鰂fnet (read-only)");
...
//			delete Buffer;
		...
		if (!CheckIndxCount())
	//		DebugMsg ("Anzahl an indx-Chunks ist ung黮tig! Datei wird als Standard-AVI behandelt");
			atType=AT_STANDARD;
		
		/*else
		{
			DebugMsg("Anzahl an indx-Chunks ist g黮tig");
		}*/
...
/*				wsprintf(Buffer,"Audiostr. %d: ID: 0x%x, %d Samples/s, %d Kan鋖e, %d Bit, %d Bytes/s, Blockalign: %d Bytes, %d komp. Bytes/s",
...
//		SuperIndex wird erst beim Schlie遝n der RIFF-AVI begonnen!
		SetPadding(2);
		free(Buffer);
...
bool AVIFILEEX::ProcessHDRL(char* lpBuffer,DWORD dwLength)
{
	...
	ZeroMemory(Buffer,sizeof(Buffer));
	sprintf(Buffer,"Streams: %d  祍pf: %d  Frames in RIFF-AVI: %d",GetNbrOfStreams(),
...
//					ASSERT(lpSubIndex);
			...
			case AVI_INDEX_OF_CHUNKS:
				...
				break;
			case AVI_INDEX_IS_DATA: break; // time-code etc. nicht unterst黷zt !
			default:
				return false;
		}
...
*/
// L鋎t Videochunk OHNE TranslateChunkNumber auszurufen!
...
/*		if ((FrameType)!=FT_DROPPEDFRAME)
		{
			...
			if (!(fDest->GetBuffer(0))) DebugMsg("  Outputbuffer ung黮tig");
			...
			switch (bResult)
			{
				// Wenn das hier nicht mehr ausgef黨rt wurde, gabs einen
